...
### UPDATE YOUR END POINT HERE - IBM Positive
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.positive%3A%22IBM%22&ts=1478569947464",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'ibm_pos' + str(count) + '.json'
...
## UPDATE YOUR END POINT HERE - IBM Neutral
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.neutral%3A%22IBM%22&ts=1478569970132",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'ibm_neu_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.negative%3A%22IBM%22&ts=1478569998089",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'ibm_neg_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
