...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.positive%3A%22Time%20Warner%22&ts=1478580223727",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'tw_pos_' + str(count) + '.json'
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.neutral%3A%22Time%20Warner%22&ts=1478580302943",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'tw_neu_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.negative%3A%22Time%20Warner%22&ts=1478580314897",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'tw_neg_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
