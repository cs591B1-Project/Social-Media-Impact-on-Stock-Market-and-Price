...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.positive%3A%22Amazon%22&ts=1478568537906",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'amazon_pos_' + str(count) + '.json'
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.neutral%3A%22Amazon%22&ts=1478568558818",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'amazon_neu_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.negative%3A%22Amazon%22&ts=1478568577553",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'amazon_neg_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
