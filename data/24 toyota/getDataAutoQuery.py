...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.positive%3A%22Toyota%22&ts=1478579048058",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'toyota_pos_' + str(count) + '.json'
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.neutral%3A%22Toyota%22&ts=1478579173199",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'toyota_neu_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
...
response = unirest.get("https://webhose.io/search?token=c6052904-f312-436b-a6d8-d915084ac866&format=json&q=language%3A(english)%20thread.country%3AUS%20organization.negative%3A%22Toyota%22&ts=1478579186864",
    headers={
    "Accept": "text/plain"
    }
)
...
while results > 0:
	...
	fileName = 'toyota_neg_' + str(count) + '.json'
	out0 = open(fileName, 'w')
	out0.truncate()
	out0.write(response.raw_body)
	out0.write("\n")
